...
# UI
ui <-
  shinyUI(navbarPage("DARAGRUB",
    selected = "Assess risk", theme = shinytheme("superhero"),
    #################### LOCAL UI ####################
    tabPanel(
      "Assess risk",
      fluidRow(
...
server <- function(input, output, session) {
  ...
  output$startStage <- renderUI({
    ...
    selectInput("startStage",
      label = h4("3. Life stage observed:"),
      choices = stageList,
  ...
  newEntry <- observe({
    if (input$update > 0) {
      ...
      isolate({
        ...
        Hpest <- values$pestcroprisk %>%
          pull(Hpest) %>%
          unlist()
        Mpest <- values$pestcroprisk %>%
          pull(Mpest) %>%
          unlist()
        df$risk <- "Low risk"
        ...
        if (nrow(pestcroprisk) > 0 && !is.na(pestcroprisk$Hcrop)) {
          croprisk <- bind_rows(
            croprisk,
            tibble(
              stage = "crop risk period", risk = "High risk", life = "crop plant",
        ...
        if (nrow(pestcroprisk) > 0 && !is.na(pestcroprisk$Mcrop)) {
          croprisk <- bind_rows(
            croprisk,
            tibble(
              stage = "crop risk period", risk = "Moderate risk", life = "crop plant",
        ...
        if (isolate(length(values$df))) {
          if (TRUE) {
            ...
            p <- ggplot(data) +
              ...
              geom_text(aes(
                x = isolate(input$startDate), label = "stage observed",
              ...
              mytheme +
              geom_hline(yintercept = length(levels(data$stage)) - 0.5, color = "white")
  ...
  output$tempresponse <- renderPlot(bg = "transparent", {
    ...
    dl <- reshape2::melt(df,
      id = "temp", variable.name = "stage",
      value.name = "dev"
    )
    p <- ggplot() +
      geom_line(data = dl, aes(x = temp, y = dev, linetype = stage, color = stage)) +
      xlab("Temperature (C)") +
      ylab("Development rate (1/d)") +
      theme(
        text = element_text(size = 20, colour = "white"), # family='Nirmala UI',
